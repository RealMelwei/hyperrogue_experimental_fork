...
int fpattern::solve() {
  ...
  for(int pw=1; pw<3; pw++) {
        ...
        for(auto& X: all_isometries) 
          if(check_order(X, rotations))
            for(auto& Y: all_isometries)
              if(check_order(Y, 2) && check_order(mmul(X, Y), hr_S3)) {
                R = X; P = Y;
                return 0;
                }
        }
    ...
    for(cs=0; cs<fmax; cs++) {
      ...
      if(!check_order(R, dual ? hr_S3 : rotations)) continue;
      ...
      for(ch=2; ch<fmax; ch++) {
        ...
        if(!check_order(mmul(P, R), dual ? rotations : hr_S3)) continue;
...
vector<triplet_info> fpattern::find_triplets() {
  ...
  for(int i: conjugacy_classes) if(gorder(i) == S7) {
    ...
    for(int j=1; j<N; j++) if(gorder(j) == 2 && gorder(gmul(i, j)) == hr_S3) {
...
int fpattern::dijkstra(vector<char>& dists, vector<int> indist[MAXDIST]) {
  ...
  for(int i=0; i<MAXDIST; i++) while(!indist[i].empty()) {
    ...
    for(int q=0; q<lg; q++) {
      ...
      else if(hr_PURE) // todo-variation: hr_PURE here?
...
#define SETDIST(X, d, it) {int c = matcode[X]; indist[d].push_back(c); if(it == itNone) ; else if(markers[c] && markers[c] != it) markers[c] = itBuggy; else markers[c] = it; }
  ...
  dijkstra(hr_PURE ? distriver : distflower, indist);
  ...
  int riverdist = dijkstra(hr_PURE ? distflower : distriver, indist);
...
EX struct fpattern& getcurrfp() {
  ...
  if(S7 == 8 && hr_S3 == 3 && !bt::in()) {
    static fpattern fp(17);
    return fp;
    }
  if(S7 == 5 && hr_S3 == 4 && !bt::in()) {
    static fpattern fp(11);
    return fp;
    }
  if(S7 == 6 && hr_S3 == 4 && !bt::in()) {
    static fpattern fp(13);
    return fp;
    }
  if(S7 == 7 && hr_S3 == 4 && !bt::in()) {
    static fpattern fp(13);
    return fp;
    }
  if(sphere || euclid) return fp_invalid;
  if(S7 == 7 && hr_S3 == 3 && !bt::in()) {
    if(hr_PURE) {
