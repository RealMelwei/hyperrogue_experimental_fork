...
EX namespace hybrid {
  ...
  EX void fixup_csteps() {
    ...
    if(!hybrid::csteps || gmod(cgi.psl_steps, hybrid::csteps)) {
      ...
      if(PIU(fake::in() ? FPIU(euclid) : euclid)) {
        ...
        if(hr_S3 == 3) hybrid::csteps *= 2;
        if(BITRUNCATED) hybrid::csteps *= hr_S3;
  ...
  EX void draw_underlying(bool cornermode) {
    ...
    hybrid::in_underlying_geometry([&] {
      ...
      if(hr_PURE && !inprod) View = View * pispin;
...
/** stretched rotation space (hr_S3 or SLR) */
