...
EX bool trees_known() {
  return sizes_known() && !(BITRUNCATED && hr_a4 && S7 <= 5);
  }
...
EX int curr_dist(cell *c) {
  ...
  switch(distance_from) {
    ...
    case dfWorld:
      ...
      return inmirror(c) ? (c->landparam & 255) : isEquidLand(c) ? c->landparam : UNKNOWNHR;
    }
  return 0;    
  }
...
void celldrawer::do_viewdist() {
  ...
  switch(number_coding) {
    case ncDistance: { 
      label = among(cd, ALTDIST_BOUNDARY, ALTDIST_UNKNOWN, ALTDIST_ERROR, UNKNOWNHR) ? "?" : its(cd);
      dc = distcolors[cd];
      // the '?' cases should have different colors
      if(cd == UNKNOWNHR) dc = distcolors[ALTDIST_BOUNDARY+1];
...
EX void set_sibling_limit() {
  ...
  #else
  else sibling_limit = hr_PURE ? 2 : 3;
  #endif
  }
