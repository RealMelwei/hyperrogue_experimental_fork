...
EX bool special_fake() {
  return fake::in() && (BITRUNCATED || (GOLDBERG && hr_S3 == 4 && gp::param.first == 1 && gp::param.second == 1) || (UNRECTIFIED && gp::param.first == 1 && gp::param.second == 1));
  }
...
void geometry_information::prepare_basics() {
  ...
  beta = (hr_S3 >= OINF && !fake::in()) ? 0 : TAU/s3;
  ...
  for(int p=0; p<100; p++) {
    ...
    if(fake::in() && WDIM == 2) {
      ...
      v2 = hdist(
        spin(90._deg/hr_S3) * xpush0(hexvdist),
        spin(-90._deg/hr_S3) * xpush0(hexvdist)
        );
  ...
  if(specfake) {
    ...
    int s6 = BITRUNCATED ? hr_S3*2 : hr_S3;
  ...
  if(mtwisted && !underlying_euclid) {
    ...
    if(0) ;
      ...
      single_step = hr_S3 * S7 - 2 * S7 - 2 * hr_S3;
      psl_steps = 2 * S7;    
      if(BITRUNCATED) psl_steps *= hr_S3;
      if(GOLDBERG && hr_S3 == 4 && gp::param == gp::loc{1,1}) psl_steps *= 2;
      if(inv) psl_steps = 2 * hr_S3;
    ...
    if(hybrid::underlying == gFake) {
      ...
      if(BITRUNCATED) fake_psl_steps *= hr_S3;
      if(inv) fake_psl_steps = 2 * hr_S3;
      if(GOLDBERG && hr_S3 == 4 && gp::param == gp::loc{1,1}) psl_steps *= 2;
  ...
  if(mtwisted && underlying_euclid) {
    ...
    #endif
    if(ug == gEuclid && hr_PURE) plevel = sqrt(3)/4.;
    if(ug == gEuclidSquare && hr_PURE) plevel = 1;
    ...
    if(ug == gEuclidSquare && fak && hr_PURE) plevel = 90._deg * fake::around - TAU;
...
EX string cgi_string() {
  ...
  if(geometry == gFieldQuotient) { V("hr_S3=", its(hr_S3)); V("S7=", its(S7)); }
  if(nil) V("NIL", its(S7));
