...
EX eMonster emerald_monster() {
  ...
  if(m == moHedge && (hr_S3 != 3 || (mhybrid && !mproduct)))
    m = moFlailer;
  return m;
  }
...
EX bool is_zebra_trapdoor(cell *c) {
  ...
  #if MAXMDIM >= 4
  else if(hr_PURE && reg3::exact_rules()) switch(geometry) {
    case gSpace534: {
  ...
  switch(c->land) {
      ...
      if(hat::in()) {
        ...
        if(d == 8 && (sphere || in_s2xe())) {
          ...
          if(gs == 3) {
            if(mhybrid) place_random_gate_continuous(c);
              ...
              c->wall = hr_PURE ? waOpenGate : waClosedGate;
            }
          ...
          else if((hrand(100) < (lookingForPrincess ? (hr_PURE ? 11 : 7) : 5) && cdist50(c)) ||
            (cdist50(c) == 0 && polarb50(c) && hrand(100) < 60)) {
            c->wall = hrand(100) < (lookingForPrincess ? (hr_PURE ? 25 : 30):50) ? waClosePlate : waOpenPlate;
            }
        ...
        if(princess::generating) {
          ...
          if(d <= 7 && c->wall == waOpenPlate && BITRUNCATED)
            c->wall = waNone;
          if(d <= 7 && c->wall == waClosePlate && hr_PURE)
            c->wall = waOpenPlate;
          // no monsters nearby
    ...
    case laEmerald: // -------------------------------------------------------------
      ...
      if(d==8) {
        ...
        else if(hr_a4 || arcm::in() || cryst || bt::in() || hr_S3 >= OINF)
      ...
      ONEMPTY {
        if(hrand(hr_PURE?400:1000) < PT(100 + 2 * (kills[moMiner] + kills[moLancer] + kills[moFlailer]), 200) && notDippingFor(itEmerald)) {
    ...
    case laWineyard:
      if(d==8) {
        ...
        else if(hr_a4 || sphere || arcm::in() || bt::in() || hr_S3 >= OINF)
    ...
    case laHell:      
      if(fargen) {
        if(hrand(1000) < (hr_PURE ? 16 : 36) && celldist(c) >= 3) {
          for(int i=0; i<c->type; i++) {
            cell *c2 = createMov(c, i);
            setdist(c2, d+1, c);
    ...
    case laHive:
      if(fargen) {
        if(hrand(2000) < (ls::tame_chaos() ? 1000 : (hr_PURE && !ls::any_chaos()) ?200: ls::any_chaos() ? 10 : ls::horodisk_structure() ? 50 : ls::hv_structure() ? 10 : 2) && !safety)
    ...
    case laRedRock:
      ...
      if(d == 7 && c->wall == waNone)
        buildPrizeMirror(c, 1000);    
      ONEMPTY {
        if(hrand_monster((doCross && celldist(c) <= 5) ?450:16000) < 30+items[itRedGem]+yendor::hardness() && !pseudohept(c) && !c->monst && !c->wall && !(!BITRUNCATED && hr_S3==4)) {
          int i = -1;
        ...
        else if(hrand_monster(16000) < 50+items[itRedGem]+yendor::hardness() && (hr_PURE?hrand(10)<3:!ishept(c)) && !c->monst)
          c->monst = moRedTroll,
          c->mondir = NODIR;
        }
      break;
    ...
    case laDocks: {
      if(d == 8) {
        if(randomPatternsMode) {
          ...
          c->wall = waSea;
          if(among(si.id, 0, 4, 16, hr_PURE ? -1 : 24))
            c->wall = waDock;
...
EX void setdist(cell *c, int d, cell *from) {
  ...
  if(hr_PURE && c->wall == waMirrorWall && c->land == laMirror)
