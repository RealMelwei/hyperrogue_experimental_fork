...
//generateEmeraldval(h);
  if(pard == 0) {
    ...
    #if CAP_GP
    else if(hr_S3 == 4 && GOLDBERG && h->c.spin(0) == S7-2 && h->move(0)->c.spin(0) >= S7-2 && h->move(0)->move(0)->s != hsOrigin) {
      heptspin hs(h, 0);
      hs += wstep;
    ...
    else if(hr_S3 == 4 && GOLDBERG && h->c.spin(0) == S7-1 && among(h->move(0)->c.spin(0), S7-2, S7-3) && h->move(0)->move(0)->s != hsOrigin) {
      heptspin hs(h, 0);
      hs += wstep;
      int d0 = hs.at->distance;
      hs += 1; hs += wstep;
    ...
    else if(hr_S3 == 4 && GOLDBERG && h->c.spin(0) >= 2 && h->c.spin(0) <= S7-2) {
    ...
    else if(h->c.spin(0) == S7-1 && hr_S3 == 4 && GOLDBERG) {
      h->distance = parent->distance + gp::dist_1();
      }
