...
bool do_kleinize() { return hr_S3 >= OINF || (cgflags & qIDEAL); }
...
void generate_matrices_scale(ld scale, int noft) {
  ...
  if(!BITRUNCATED) {
    ...
    generate_matrices(hex_matrices, ohex, msh(geometry, S6, cgi.hexvdist, cgi.hexhexdist, cgi.hcrossf, (hr_S3-3)*M_PI/hr_S3, scale));
...
void geometry_information::generate_floorshapes_for(int id, cell *c) {
  ...
  else if(hr_PURE && geometry != gBinaryTiling && geosupport_football() < 2) {
    siid = 1;
    }

  else if(hr_PURE && !(S7&1) && !aperiodic && !hr_a4) {
  ...
  for(auto pfsh: all_escher_floorshapes) {
    ...
    if(STDVAR && standard_tiling()) {
      for(auto ii: {0, 1}) {
        ...
        if(hr_PURE && geosupport_football() < 2) {
